...
const MasculinePage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        let filterStr = 'category="Masculino"';
        ...
        console.error('[MasculinePage] Error fetching products:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      <Helmet>
        <title>Perfumes Masculinos | Velour Perfumes</title>
        <meta name="description" content="Descubra nossa coleção exclusiva de perfumes masculinos. Fragrâncias marcantes das melhores marcas do mundo." />
      </Helmet>

      {/* Hero Section */}
      <section className="relative py-16 md:py-24 overflow-hidden border-b border-amber-900/20">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-amber-900/20 via-background to-background pointer-events-none"></div>
        <div className="container mx-auto px-4 relative z-10">
          ...
          >
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-amber-100 mb-6 tracking-tight">
              Perfumes <span className="text-amber-500">Masculinos</span>
            </h1>
            <p className="text-lg md:text-xl text-amber-100/70 leading-relaxed max-w-2xl">
              Fragrâncias intensas e sofisticadas para homens que deixam sua marca por onde passam. Explore nossa seleção premium.
            </p>
          </motion.div>
        </div>
      </section>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center bg-zinc-900/30 rounded-2xl border border-amber-900/20">
            ...
            <p className="text-amber-100/60 max-w-md">
              Não encontramos perfumes masculinos para a marca selecionada no momento. Tente selecionar outra marca.
            </p>
            <button 
              onClick={() => setSelectedBrand('All')}
