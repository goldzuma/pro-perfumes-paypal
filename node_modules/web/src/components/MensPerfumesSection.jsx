    ...
    const fetchProducts = async () => {
      try {
        ...
        const result = await pb.collection('products').getFullList({ 
          filter: 'category="Masculino"',
          sort: '-created',
          expand: 'brand', // Expand brand para obter nome da marca
          $autoCancel: false
        });
        
        console.log(`[MensPerfumesSection] SUCESSO: ${result.length} produtos masculinos carregados sem limite.`);
        setProducts(result);
      } catch (err) {
        console.error('[MensPerfumesSection] Erro ao buscar produtos:', err);
        setError(`Não foi possível carregar a coleção masculina. Detalhes: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="container mx-auto max-w-7xl">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          ...
          >
            ...
            <p className="text-amber-100/70 text-lg max-w-xl font-light">
              Poder, sofisticação e presença em cada gota.
            </p>
          </motion.div>
          ...
          <motion.div
            ...
            transition={{ duration: 0.6 }}
          >
            <Link to="/catalog?category=Masculino">
        ...
        {error ? (
          ...
          <div className="text-center py-12 text-amber-100/60">
            <p>Nenhum perfume masculino encontrado no momento.</p>
          </div>
        ) : (
...
export default MensPerfumesSection;
